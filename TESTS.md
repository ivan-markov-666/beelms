...
Test Service е отговорен за управлението на тестове, въпроси и опити за тестване в платформата. Той включва следните основни функционалности:

- Създаване и управление на тестове
- Управление на въпроси в тестове
- Проследяване на опитите за тестване
...
1. **Unit тестове**
   - Тестват отделни компоненти в изолация
   - Намират се в `src/**/*.spec.ts`
   - Изпълняват се с `npm run test`

2. **Интеграционни тестове**
   - Тестват взаимодействието между компоненти
   - Намират се в `test/integration/`
   - Изпълняват се с `npm run test:integration`

3. **E2E тестове**
   - Тестват цялостната функционалност
   - Намират се в `test/e2e/`
   - Изпълняват се с `npm run test:e2e`
...
- Създаване на тест
- Извличане на тест по ID
- Актуализиране на тест
- Изтриване на тест

##### Управление на въпроси
- Добавяне на въпрос към тест
- Извличане на въпроси за тест
- Актуализиране на въпрос
- Изтриване на въпрос

##### Тестови опити
- Стартиране на тестов опит
...
Тестовите данни се намират в `test/data/` и включват:
- `tests/` - Примерни тестове
- `questions/` - Примерни въпроси
- `attempts/` - Примерни опити за тестване
...
# Изпълнение на E2E тестове
npm run test:e2e

# Генериране на отчет за покритието
npm run test:cov
```

#### Допълнителни инструменти

- **ESLint** - За проверка на кодовия стил
- **Prettier** - За форматиране на кода
- **Husky** - За изпълнение на тестове преди commit

#### Документация

- [API документация](http://localhost:3000/api) (при стартиран сървър за разработка)
- [Swagger документация](http://localhost:3000/api-docs) (ако е конфигурирана)
...
### Тестове на Course Service

#### Общ преглед

Тестовете за Course Service са разпределени в три основни тестови файла, които покриват различни нива на тестване на функционалността за управление на курсове.

#### Тестови файлове

1. **app.controller.spec.ts**
   - Тества основния контролер на приложението
   - Съдържа тест за основния endpoint, който връща "Hello World!"
   - Използва мокнати зависимости за изолирано тестване

2. **course.controller.spec.ts**
   - Тества функционалността на CourseController
   - Включва тестове за всички HTTP ендпойнти
   - Проверява валидацията на входните данни
   - Тества правилното извикване на методите от CourseService

3. **course.service.spec.ts**
   - Тества бизнес логиката в CourseService
   - Включва unit тестове за всички основни методи
   - Тества обработката на грешки и изключения
   - Проверява взаимодействието с базата данни

#### Как да изпълните тестовете

##### Изпълняване на всички тестове в услугата:

```bash
# От директорията на услугата
cd services/course
npm test
```

##### Изпълняване на конкретен тестов файл:

```bash
# За тестове на контролера
npm test src/course/course.controller.spec.ts

# За тестове на сървиса
npm test src/course/course.service.spec.ts
```

##### Изпълняване с гледане на промените:
```bash
npm test -- --watch
```
...
Тестовете трябва да покриват поне:
- 80% от кода на сървиса
- Всички основни сценарии за успешно изпълнение
- Основните сценарии за грешки
- Всички публични методи и ендпойнти

## Тестове на Ads Service

### Общ преглед

Тестовете за Ads Service са разработени да гарантират коректната работа на функционалностите за управление на реклами и събиране на статистика. Те включват unit тестове за сървиси и контролери, както и интеграционни тестове.

### Тестови файлове

1. **ads.controller.spec.ts**
   - Тества HTTP ендпойнтите на контролера
   - Валидира входните параметри и изходните данни
   - Проверява автентикацията и оторизацията

2. **ads.service.spec.ts**
   - Тества бизнес логиката на услугата
   - Покрива сценарии за управление на реклами
   - Включва тестове за статистическите изчисления

3. **app.controller.spec.ts**
   - Тества основните ендпойнти на приложението
   - Проверява здравословното състояние на услугата

### Ключови тестови сценарии

#### Управление на реклами
- Създаване на нова реклама (валидна и невалидна входна информация)
- Извличане на реклами с различни филтри
- Актуализиране на съществуваща реклама
- Изтриване на реклама

#### Статистика и проследяване
- Проследяване на показвания на реклами
- Записване на кликвания върху реклами
- Генериране на статистически отчети
- Агрегиране на данни за използване

#### Интеграция с Redis
- Кеширане на често използвани заявки
- Управление на сесии
- Избягване на състезателни условия
...
#### Изпълняване на конкретни тестове:

```bash
# Само unit тестове
npm test src/ads/ads.service.spec.ts

# Само контролер тестове
npm test src/ads/ads.controller.spec.ts

# Следене на промени
npm test -- --watch
```

#### Изпълнение с покритие на кода:
```bash
npm run test:cov
```
...
Тестовите данни се генерират автоматично с помощта на `@faker-js/faker` и включват:
- Тестови рекламни кампании
- Потребителски сесии
- Статистически данни за показвания и кликвания

### Интеграция с регресионния тестов пакет

Тестовете могат да се изпълнят и като част от регресионния тестов пакет:
