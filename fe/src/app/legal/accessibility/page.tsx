...
async function fetchAccessibility(lang: SupportedLang): Promise<LegalPageDto> {
  const query = lang ? `?lang=${encodeURIComponent(lang)}` : "";
  const res = await fetch(buildApiUrl(`/legal/accessibility${query}`), {
    next: { revalidate: 60 },
  });
  ...
  if (!res.ok) {
    throw new Error("Failed to load Accessibility statement");
  }
...
export default async function AccessibilityPage({
  ...
  const page = await fetchAccessibility(lang);
