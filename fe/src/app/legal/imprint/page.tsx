...
async function fetchImprint(lang: SupportedLang): Promise<LegalPageDto> {
  const query = lang ? `?lang=${encodeURIComponent(lang)}` : "";
  const res = await fetch(buildApiUrl(`/legal/imprint${query}`), {
    next: { revalidate: 60 },
  });
  ...
  if (!res.ok) {
    throw new Error("Failed to load Imprint");
  }
...
export default async function ImprintPage({
  ...
  const page = await fetchImprint(lang);
