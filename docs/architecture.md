...
#### 5.1. Глобална CORS Конфигурация

За да се осигури сигурна, но гъвкава комуникация между front- и back-end, NestJS приложението активира **CORS** със следните принципи:

- **Динамичен списък с origin-и**: Допустимите origin-и се задават чрез env променлива `CORS_ORIGIN` (списък, разделен със запетая). В development може да се зададе `*`.
- **Allow Credentials**: `credentials: true`, тъй като приложението използва `httpOnly` бисквитки за `refresh_token`.
- **Разрешени методи**: `GET,HEAD,PUT,PATCH,POST,DELETE`.
- **Сигурност**: В production е силно препоръчително `CORS_ORIGIN` да съдържа само конкретни домейни.

Примерен код (виж `main.ts`):
```ts
app.enableCors({
