...
## Dev Tasks
- [ ] Имплементация на Admin layout компонент (shared, ако е нужно).
- [ ] Добавяне на маршрути `/admin` и (placeholder) `/admin/wiki` в Next.js app-а.
- [ ] FE guard логика:
  - [ ] използва данните от WS-2 Auth (token/role) за да определи дали потребителят е admin;
  - [ ] при липса на валиден токен – redirect към `/auth/login`;
  - [ ] при логнат, но non-admin профил – рендър на страница „Access denied“ (HTTP 403 стил) без допълнителен redirect.
- [ ] Интеграция с глобалната навигация:
  - [ ] Admin линк/бутон се показва само за admin потребител (напр. в профил менюто).
- [ ] FE тестове:
  - [ ] рендер на `/admin` за admin потребител;
  - [ ] проверка, че потребител без валиден токен се пренасочва към `/auth/login`;
  - [ ] проверка, че non-admin вижда страница „Access denied“ (403 стил) и няма достъп до съдържанието на Admin зоната;
  - [ ] проверка, че Admin навигацията съдържа линк към `/admin/wiki` за admin и че Admin линк/бутон не се визуализира за non-admin в глобалната навигация.
