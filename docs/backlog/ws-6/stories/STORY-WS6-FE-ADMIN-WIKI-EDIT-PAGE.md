...
## Dev Tasks
- [x] Дефиниране/уточняване на route за Admin Wiki edit страницата в Next.js приложението (напр. `/admin/wiki/[id]/edit` или подобна структура).
- [x] Имплементация на клиентския компонент за формата:
  - [x] Извличане на данните за статията от Admin Wiki API при mount;
  - [x] Управление на локално състояние (формови полета, loading/error флагове);
  - [x] Submit логика, която вика `PUT /api/admin/wiki/articles/{id}` с новите стойности.
- [x] Интеграция с Admin layout-а и навигацията (линк "Редактирай" от списъка `/admin/wiki`, когато това бъде добавено).
- [x] FE тест(ове) с React Testing Library:
  - [x] рендер на страницата в admin контекст с mock-нат fetch за първоначално зареждане;
  - [x] сценарий за успешен save (mock 200) и проверка за съобщение за успех;
  - [x] сценарий за грешка (mock 400/500) и проверка за съобщение за грешка.
