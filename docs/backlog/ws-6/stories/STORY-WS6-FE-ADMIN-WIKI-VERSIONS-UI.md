...
## Dev Tasks
- [ ] Дизайн/структуриране на Versions секцията в Admin Wiki edit UI (таб, sidebar или отделна страница), съобразено с UX документа.
- [ ] Имплементация на компонент за списък с версии:
  - [ ] fetch към `GET /api/admin/wiki/articles/{id}/versions` при зареждане;
  - [ ] състояния loading/empty/error за списъка;
  - [ ] визуализация на основните полета за всяка версия.
- [ ] Имплементация на rollback действие:
  - [ ] confirm диалог/прост `window.confirm` за потвърждение;
  - [ ] извикване на `POST /api/admin/wiki/articles/{id}/versions/{versionId}/restore`;
  - [ ] обновяване на текущото съдържание/формата при успех;
  - [ ] показване на съобщения за успех/грешка.
- [ ] FE тест(ове) с React Testing Library:
  - [ ] рендер на списъка с mock-нат отговор от versions endpoint-а;
  - [ ] проверка, че бутон/линк за rollback присъства за всяка версия;
  - [ ] сценарий за успешен rollback (mock 200) и показване на съобщение за успех;
  - [ ] сценарий за грешка (mock 404/500) и показване на съобщение за грешка.
