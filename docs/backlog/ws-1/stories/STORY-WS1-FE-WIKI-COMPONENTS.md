...
## Dev Tasks
- [x] Дефиниране на малка FE структура за Wiki компоненти (напр. `fe/src/app/wiki/_components/`):
...
- [x] Рефактор на `/wiki` да използва новите компоненти без промяна в behavior-а и текстовете.
- [x] Рефактор на `/wiki/[slug]` (вкл. `loading.tsx`, `error.tsx`, `not-found.tsx`) да използва новите компоненти без промяна в behavior-а и текстовете.
- [x] Актуализиране/допълване на съществуващите FE тестове (`wiki-page.test.tsx`, `wiki-article-page.test.tsx`) да отразяват новата компонентна структура, като се уверим, че WS-1 флоу остава зелен.
