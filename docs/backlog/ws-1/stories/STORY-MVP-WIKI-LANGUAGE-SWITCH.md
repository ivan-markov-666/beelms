...
## Dev Tasks
- [x] **Frontend**
  - [x] Имплементиране/актуализиране на global language switcher в header-а според UX Design.
  - [x] Въвеждане на механизъм за съхранение на избрания език чрез `lang` query параметър в URL (в синхрон с текущата архитектура и mini tech-spec за EPIC-CROSS-I18N).
  - [x] Адаптиране на `/wiki` и `/wiki/[slug]` да използват избрания език при извличане и визуализация на съдържание.
- [x] **Backend**
  - [x] `GET /api/wiki/articles` и `GET /api/wiki/articles/{slug}` поддържат параметър за език според DB модела и OpenAPI (покрито от по-ранните Wiki BE stories, без нужда от допълнителни промени в това story).
  - [x] Тестове за различни комбинации: статии с една езикова версия, с няколко версии, без версия на избрания език (unit/e2e тестовете за WikiService и Wiki API покриват тези сценарии).
- [x] **Тестове**
  - [x] Ръчно тестване на основния flow: `/wiki` → смяна на език → избор на статия → `/wiki/[slug]` → смяна на език.
  - [x] FE тестове за `/wiki`, `/wiki/[slug]` и `LanguageSwitcher`, които проверяват предаването и поведението на `lang` параметъра.
