...
- [x] **Backend (NestJS API)**
  - [x] Имплементиране/доуточняване на поддръжката на `page` и `pageSize` в `GET /api/wiki/articles` в съответствие с `docs/architecture/openapi.yaml`.
  - [x] Осигуряване на стабилен default (напр. `page = 1`, `pageSize = 20`) при липсващи параметри.
  - [x] Тестове за различни сценарии: `page = 1`, средна страница, последна страница, извън обхват, комбинация със `q` и `lang`.
- [x] **Frontend (Next.js)**
  - [x] Добавяне на UI за pagination на `/wiki` (бутон Next/Previous или номерирани страници) според UX Design.
  - [x] Връзка между UI pagination и query параметрите към API (`page`, `pageSize`), включително encode в URL (напр. `?page=2`).
  - [x] Осигуряване на коректна работа заедно със Search/Language filter (ако са налични).
  - [x] Ръчно тестване на сценарии: малко статии (1 страница), много статии (няколко страници), комбинация с търсене/филтър, опит за достъп до извън-обхват страница.
- [x] **Тестове**
  - [x] Базови FE/интеграционни тестове (или ръчен чеклист) за пагинацията на `/wiki`.
  - [x] BE unit/integration тестове за логиката на pagination в `WikiService`/репозиториото.
...
- В момента пагинацията се прилага на ниво статии преди филтриране по `q` и `lang`. За по-големи данни бъдеща версия може да премести филтрирането в самата DB заявка (WHERE + ORDER BY) и да пагинира вече филтрирания резултат.
- Публичният API в момента връща само масив от статии. За по-богато UX поведение може да се добави отговор тип „envelope“ с метаданни (`items`, `totalCount`, `page`, `pageSize`, `hasNextPage`), така че FE да не разчита на евристика (`articles.length === pageSize`).
