...
## Dev Tasks
- [x] Дефиниране на TypeORM entity класове за `WikiArticle` и `WikiArticleVersion` според `docs/architecture/db-model.md`.
- [x] Създаване на миграции за съответните таблици, индекси и релации.
- [x] Имплементиране на seed механизъм (напр. отделен скрипт или миграция), който вмъква примерните статии.
- [x] Тестване на миграциите и seed-a в чиста локална база (up/down сценарии).
- [x] Ръчно потвърждение (чрез SQL/ORM или API), че статии се зареждат коректно и са достъпни през API ендпойнтите.
