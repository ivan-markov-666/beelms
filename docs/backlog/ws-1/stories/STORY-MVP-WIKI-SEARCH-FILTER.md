...
## Dev Tasks
- [x] **Frontend (Next.js)**
  - [x] Добавяне на Search поле и Language filter UI в `SCR-WIKI-LST` (`/wiki`), със стилове от design system-а.
  - [x] Имплементиране на логика за прилагане на Search + Language filter към извикването на API (`GET /api/wiki/articles`), включително encode-ване на заявката в query параметри.
  - [x] Имплементиране на „no results“ state (визуално и текстово), координирано с `STORY-WS1-FE-WIKI-STATES`.
  - [x] Ръчно тестване на комбинации: без филтри, само език, само търсене, език + търсене, без резултати.
- [x] **Backend (NestJS API)**
  - [x] Разширяване на `GET /api/wiki/articles` да приема опционални query параметри за търсене и език, в синхрон с OpenAPI спецификацията.
  - [x] Имплементиране на филтриране върху активните статии по подадените критерии (full-text/ILIKE върху заглавие/ключови думи, филтър по език/езикова версия).
  - [x] Тестове за API поведението: без филтри, само език, само търсене, без резултати.
- [x] **Тестове**
  - [x] Базови FE/интеграционни тестове (или ръчен чеклист) за Search + Language filter на `/wiki`.
  - [x] BE unit/integration тестове за новото филтриране в `WikiService`/репозиториото.
