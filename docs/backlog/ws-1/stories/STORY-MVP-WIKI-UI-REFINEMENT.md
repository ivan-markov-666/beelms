...
## Dev Tasks
- [ ] Преглед на UX документа `docs/ux/qa4free-ux-design.md` за секциите, свързани с Wiki (SCR-WIKI-LST, SCR-WIKI-ART) и глобалния layout.
- [ ] Актуализиране на FE компонентите за `/wiki` и `/wiki/[slug]` (вкл. споделените Wiki компоненти от `STORY-WS1-FE-WIKI-COMPONENTS`), така че да следват по-точно описания UX (типография, spacing, цветове, поведение на линковете и състоянията).
- [ ] Преглед и корекция на `loading`, `error` и `not-found` състоянията за двете страници, така че да са визуално консистентни, без да се променя функционалният им обхват.
- [ ] Тестване на desktop и mobile изгледи (ръчно, в браузър) за основните сценарии: списък със статии, празен списък, 404 статия, грешка при зареждане.
