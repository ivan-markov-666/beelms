...
- Важно е съобщенията да не издават дали даден имейл съществува в системата.
 - FE имплементацията е вързана към общия FE i18n слой (`EPIC-CROSS-I18N`): екраните `/auth/forgot-password` и `/auth/reset-password` използват споделените i18n helper-и и показват етикети, бутони и съобщения на BG/EN според глобалния `lang` параметър от header `LanguageSwitcher`.
