...
- В бъдещи WS може да се добавят допълнителни удобства (запазване на имейл, „remember me“, визуални индикатори за сила на паролата и др.), но за WS-2 целта е минимален, но използваем флоу.
 - FE имплементацията е вързана към общия FE i18n слой (`EPIC-CROSS-I18N`): екраните `/auth/register` и `/auth/login` използват споделените i18n helper-и и показват етикети, бутони и съобщения на BG/EN според глобалния `lang` параметър от header `LanguageSwitcher`.
