...
- Смяна на имейл (`PATCH /api/users/me`):
  ...
  - За да се предотврати злоупотреба, потвържденията на смяна на имейл през `pendingEmailVerificationToken` се ограничават до максимум **3 успешни верификации в рамките на 24 часа**. При опит за допълнителни потвърждения в този прозорец BE връща `429 Too Many Requests` с ясно съобщение ("email change verification limit reached"). След изтичане на 24 часа прозорецът и броячът се нулират.
- Security / GDPR:
...
- В рамките на WS-2 е добавен прост rate limiting за потвържденията на смяна на email (3 успешни потвърждения за 24 часа), така че системата да остане защитена от прекомерни заявки, без да усложнява първоначалната архитектура.
