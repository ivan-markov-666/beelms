...
- [ ] Имплементиране на UI за промяна на email адреса и интеграция с `PATCH /api/users/me` (валидиране, показване на съобщения за успех/грешка).
- [ ] Имплементиране на UI за смяна на паролата (текуща/нова/потвърждение) и интеграция с `POST /api/users/me/change-password`.
