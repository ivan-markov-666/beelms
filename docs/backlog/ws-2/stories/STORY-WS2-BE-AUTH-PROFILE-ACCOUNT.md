...
- `GET /api/users/me`:
  ...
  - Профилният отговор включва и флаг/метаданни за лимита на смяна на email (напр. `emailChangeLimitReached`, `emailChangeLimitResetAt`), когато функционалността за лимит е активна, така че FE да може да визуализира статуса към потребителя.
...
- `PATCH /api/users/me` (актуализиране на базова профилна информация):
  ...
  - Когато е достигнат конфигурираният лимит за потвърждения на смяна на email (напр. 3 успешни потвърждения за 24 часа), API може да връща `429 Too Many Requests` с ясно съобщение, което FE препредава към потребителя.
...
## Dev Tasks
- [x] Имплементиране на `GET /api/users/me` с извличане на данни за текущия потребител от контекста/JWT.
- [x] Имплементиране на `PATCH /api/users/me` за актуализиране на базова профилна информация (поне email) с валидация и проверка за дублиран имейл.
- [x] Имплементиране на `POST /api/users/me/change-password` с проверка на текущата парола, валидация на новата парола и обновяване на паролния hash.
- [x] Имплементиране на `DELETE /api/users/me` с реална логика за изтриване/анонимизиране на данните в съответствие с GDPR.
- [x] Имплементиране на `POST /api/users/me/export` с генерация на файл/отговор с личните данни и интеграция с anti-bot защита.
- [x] Актуализиране на модели/миграции, ако са нужни допълнителни полета за GDPR/track-ване.
- [x] Unit тестове за всички по-горе описани ендпойнти (GET, PATCH, change-password, DELETE, export), покриващи нормални и гранични сценарии, включително security правила (non-enumeration, валидация на парола, обработка на токени).
- [x] Интеграционни тестове за end-to-end поведение на смяна на парола, изтриване и експорт на акаунт.
- [x] Ръчно тестване чрез Swagger UI/Postman и впоследствие през FE екраните за профил.
