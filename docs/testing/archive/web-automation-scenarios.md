...
Целта е да имаме ясен списък със сценарии, които по-късно да автоматизираме, след като MVP версията / тестовия framework е готов.
...
- **Проверки (REST):**
  - `GET /api/users/me` с токена, който UI е записал → връща коректен профил (id, email, createdAt, role).
