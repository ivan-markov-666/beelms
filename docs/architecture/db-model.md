...
- На база на `TaskResult` могат да се изчисляват метрики в `/api/admin/metrics/overview`.
...
- Метриките могат да се изчисляват динамично върху `User`, `Task`, `TaskResult` (или да се добавят агрегиращи таблици в по-късен етап).
