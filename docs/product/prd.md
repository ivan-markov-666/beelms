...
- FR-CROSS-3: Събиране на базови метрики за потребителско поведение и производителност на системата. Метрики за индивидуален учебен напредък (завършени задачи/exams) не са част от текущото MVP и ще се добавят в бъдещия „exams“ модул.
  - Администраторските екрани (напр. Admin Metrics Overview) показват **само агрегирани метрики** (брой сесии, средна продължителност, разпределение по източник на трафик и др.), без индивидуално ниво на потребител. По избор може да има филтър за период (напр. последни 7/30 дни) и export на тези агрегирани данни (CSV/JSON).
